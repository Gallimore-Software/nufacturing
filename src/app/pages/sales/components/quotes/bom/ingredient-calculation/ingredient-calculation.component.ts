...
export class IngredientCalculationComponent implements OnInit {
  ...
  initIngredients(): void {
    ...
    ingredientsData.forEach(ingredient => {
      this.ingredients.push(this.fb.group({
        ...
        nfgTargetCapsule: [this.calculateNfgTargetCapsule(ingredient.perCapsule)],
        perBottle: [50],
  ...
  calculateSummary(): void {
    ...
    this.ingredients.controls.forEach(control => {
      ...
      const percentOfTotalKg = (totalKgBeforeWaste > 0) ? (conversionToKg / totalKgBeforeWaste) * 100 : 0;
      ...
      const extraMgToBottle = (mgPerBottle > 0) ? extraKgOfWasteMg / mgPerBottle : 0;
      const intentionalOverages = (conversionToKg > 0) ? ((extraKgOfWasteMg / conversionToKg) * 100) - 100 : 0;
    ...
    this.nfgCapsuleFillRange = { min: this.toFixed(minFillRange, 4), max: this.toFixed(maxFillRange, 4) };
