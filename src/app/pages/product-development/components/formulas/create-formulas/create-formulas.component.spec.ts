...
describe('CreateFormulasComponent', () => {
  ...
  it('should create a formula when the form is valid and save is clicked', () => {
    ...
    const activeIngredientsArray = component.formulaForm.controls['activeIngredients'] as FormArray;
    activeIngredientsArray.clear(); // Clear any existing controls
    activeIngredientsArray.push(formBuilder.group({
      name: 'Melatonin',
    ...
    const inactiveIngredientsArray = component.formulaForm.controls['inactiveIngredients'] as FormArray;
    inactiveIngredientsArray.clear(); // Clear any existing controls
    inactiveIngredientsArray.push(formBuilder.group({
      name: 'Gelatin',
    ...
    expect(dialogRefSpy.close).toHaveBeenCalledWith(expect.objectContaining({
      name: 'Sleep Aid Formula',
      productType: 'Gummies',
      unitOfMeasurement: 'mcg',
      activeIngredients: [
        { name: 'Melatonin', scientificName: 'N-Acetyl-5-Methoxytryptamine', perUnit: 5 },
        { name: 'Valerian Root', scientificName: 'Valeriana Officinalis', perUnit: 50 },
      ],
      inactiveIngredients: [
        { name: 'Gelatin', scientificName: 'Gelatin', perUnit: 500 },
        { name: 'Natural Flavors', scientificName: 'Natural Flavors', perUnit: 20 },
