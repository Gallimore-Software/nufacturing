...
import { MAT_DIALOG_DATA, MatDialog, MatDialogRef } from '@angular/material/dialog'; // Import MatDialog for opening dialogs
...
export class BatchDetailDialogComponent implements OnInit {
  ...
  constructor(
    @Inject(MAT_DIALOG_DATA) public data: { batch: BatchRecord; assignable: boolean },
  ...
  saveBatchDetails(): void {
    if (this.batchForm.valid && this.isAdminOrManager && this.data.assignable) {
      ...
      this.batchRecordsService.updateBatchRecord(this.data.batch._id, updatedBatch).subscribe(() => {
